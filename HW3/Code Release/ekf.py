...
class Localization_EKF(EKF):
    ...
    def map_line_to_predicted_measurement(self, m):
        ...
        ##############
        x, y, Th = self.x
        A = alpha - Th - self.tf_base_to_camera
        R = r -np.sqrt((x*np.cos(A))**2 + (y*sin(A))**2)

        dAdx = 0
        dAdy = 0
        dAdTh = -1
        dRdx = -( 1/(2*np.sqrt((x*np.cos(A))**2 + (y*np.sin(A))**2)) ) * 2*x*np.cos(A)**2
        dRdy = -( 1/(2*np.sqrt((x*np.cos(A))**2 + (y*np.sin(A))**2)) ) * 2*y*np.sin(A)**2
        dRdTh = -((2*x*np.cos(A) + 2*y*sin(A))/(2*np.sqrt((x*np.cos(A))**2 + (y*np.sin(A))**2))) * (-np.sin(A) + np.cos(A))
...
#        self.map_lines = map_lines                    # 2xJ matrix containing (alpha, r) for each of J map lines
            for j in range(self.map_lines.shape[1]):
                m = (self.map_lines[:,j][0],self.map_lines[:,j][1])
            ...
            v_list = [v_listA[i] for i in range(len(v_listA)) if v_listA[i] < self.g]
            R_list = [R_listA[i] for i in range(len(R_listA)) if v_listA[i] < self.g]
            H_list = [H_listA[i] for i in range(len(H_listA)) if v_listA[i] < self.g]
    ...
    def measurement_model(self, rawZ, rawR):
        ...
        for i in range(len(R_list)):
            arr[2*i:2+2*i,2*i:2+2*i] = R_list[i]        
